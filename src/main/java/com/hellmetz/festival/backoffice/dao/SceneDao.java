...
public class SceneDao {  // ✅ pas de static
        ...
        try (Connection cn = ConnectionFactory.getConnection();
            ...
            while (rs.next()) {
                Scene scene = new Scene(
                        rs.getInt("id_scene"), rs.getString("nom_scene"),
                        rs.getString("description"), rs.getInt("capacite"),
                        rs.getBoolean("actif"), rs.getString("type_scene"),
                        rs.getInt("superficie_m2"), rs.getString("url_plan_technique"));
                result.add(scene);
            }
    ...
    public void insert(Scene scene) {
        // ✅ INSERT INTO scene (pas groupe), et 7 colonnes = 7 points d'interrogation
    ...
    public void update(Scene scene) {
        ...
        try (Connection cn = ConnectionFactory.getConnection();
            ...
            fillPreparedStatement(ps, scene);
            ps.setInt(8, scene.getId());  // ✅ paramètre 8, pas 17
            ps.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
